...
import random
import pymysql
from crm_style.function import get_fileBasePath
import xlsxwriter
...
def getRandomName():
    ...
    firstNamePath=r'C:\Users\Administrator\Desktop\firstname.txt'
    midleNamePath=r'C:\Users\Administrator\Desktop\midname.txt'
    lastNamePath=r'C:\Users\Administrator\Desktop\lastname.txt'
    firstname = []
    midlename = []
    lastname = []
...
def nameToMySQL():
    ...
    db = pymysql.Connect(
        host='192.168.37.113', port=3306, user='opm_sh_sit', passwd='opm_sh_sit', db='test', charset='utf8')
    ...
    for a in range(len(fullname)):
        ...
        cursor.execute("insert into %s(name) values('%s')"%('auto_name',fullname[a]))
        db.commit()
